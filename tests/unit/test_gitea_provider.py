...
def test_get_default_owner(gitea_provider):
    ...
    with patch("soliplex.agents.scm.gitea.settings") as mock_settings:
        mock_settings.gitea_owner = "gitea-owner"
...
def test_get_auth_token(gitea_provider):
    ...
    with patch("soliplex.agents.scm.gitea.settings") as mock_settings:
        mock_settings.gitea_token = "gitea-token-123"
...
def test_get_last_updated(gitea_provider):
    ...
    rec = {"name": "test.md", "last_committer_date": "2024-01-15T12:00:00Z", "sha": "abc123"}
...
def test_initialization_with_default_owner(gitea_provider):
    ...
    with patch("soliplex.agents.scm.gitea.settings") as mock_settings:
        mock_settings.gitea_owner = "default-gitea-owner"
...
def test_build_url(gitea_provider):
    ...
    with patch("soliplex.agents.scm.gitea.settings") as mock_settings:
        mock_settings.gitea_url = "https://gitea.example.com/api/v1"
        mock_settings.gitea_owner = "test-owner"
